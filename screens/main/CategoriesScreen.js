...
const categories = [
  ...
  { title: "Food & Snacks", icon: "fast-food-outline", screen: "FoodSnacksScreen" },
...
export default function CategoriesScreen({ navigation }) {
  return (
    <View style={styles.container}>
      <Header title="Categories" navigation={navigation} onBack={() => navigation.goBack()} />
        ...
        <View style={styles.grid}>
          {categories.map((cat, index) => (
            <TouchableOpacity
              key={index}
              style={styles.card}
              onPress={() => navigation.navigate(cat.screen, { products })}
            >
